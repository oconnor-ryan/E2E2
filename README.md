...
### X3DH Key Exchange
In order to allow asyncronous messages to be sent, I decided to implement the [Extended Triple Diffie-Hellman (X3DH)](https://signal.org/docs/specifications/x3dh/) key exchange algorithm from the Signal Protocol. This algorithm was developed by a software development group known as Open Whisper Systems, who later developed the Signal Messanger application. 
...
### Starting A Call
Once a user chooses who they want to call, they must make an offer by sending a [Session Description Protocol (SDP)](https://developer.mozilla.org/en-US/docs/Glossary/SDP) string to the callee using the WebSocket connection.

Once the callee receives this SDP, they will provide an answer using their own SDP string.
After this, both users will send a list of [Interactivity Conneectivity Establishment (ICE)](https://developer.mozilla.org/en-US/docs/Glossary/ICE) candidates, which are essentially a set of potential IP addresses, ports, and protocols (TCP or UDP) that each peer can use to establish a connection. More information about ICE can be found here:
